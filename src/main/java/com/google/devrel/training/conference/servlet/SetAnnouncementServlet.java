...
public class SetAnnouncementServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Iterable<Conference> iterable =ofy().load().type(Conference.class).filter("seats<", 5).filter("seats>", 0);
        ...
        if (conferenceNames.size() > 0) {
            ...
            String str = announcementStringBuilder.toString();
            memcacheService.put(key, str);
        }
